...
def solve_LP_Row_Generation(Graph: networkx.DiGraph, T: int, v_best: float, s, t) -> float:
    """Solves the MPC TRF problem on a given graph with given time horizon T and minimum flow value v_best via a LP with dynamic Row Generation. Prints the used path with flow rate and total flow and returns the peak costs
    ...
    while model.Status != GRB.INFEASIBLE:
        ...
        if total_cost - 1e-9 > C_Max_val:
            expr = gurobipy.LinExpr()
            for i,path in enumerate(paths):
                if f[i] is not None:
                    expr += f[i] * calc_cost(Graph, path, theta, T)
            
            model.addConstr(expr <= C_Max)
            added = True
        ...
        if invalid_edge is not None:
            model.addConstr(quicksum(f[i] for i,path in enumerate(paths) if f[i] is not None and any((path[j],path[j+1]) == (invalid_edge[0], invalid_edge[1]) for j in range(len(path)-1))) <= Graph[invalid_edge[0]][invalid_edge[1]]['cap'])
            added = True
        
        #if no constraint is added, the solution is valid
        if not added:
            break
    end_time_optimization = time.time()
    print('Runtime model optimization:', end_time_optimization - start_time_optimization)
    end_time_all = time.time()
    print('Runtime LP Row Generation Overall:', end_time_all-start_time_all)

    if model.Status != GRB.INFEASIBLE:
        print('Peak-Cost LP Row Generation:', model.ObjVal)
        ...
        print('Peak-Cost LP Row Generation: INFEASIBLE')
